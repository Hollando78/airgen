...
import { tryGeneratePreviewPdf } from "./documents/helpers/file-upload-helpers.js";
import { parseMultipartFormData } from "./documents/helpers/multipart-parser.js";
import { buildDownloadUrl, buildPreviewUrl } from "./documents/helpers/document-url-builders.js";
import { ensureUniqueDocumentSlug } from "./documents/helpers/slug-helpers.js";
...
export default async function registerDocumentRoutes(app: FastifyInstance): Promise<void> {
  ...
  // POST /surrogates - Create a new surrogate reference
  app.post("/surrogates", {
    schema: {
      tags: ["surrogates"],
  ...
  // POST /sections/:sectionId/reorder-infos
  ...
  // POST /sections/:sectionId/reorder-surrogates
  ...
  // POST /sections/:sectionId/reorder-requirements
  ...
  // POST /sections/:sectionId/reorder-with-order
  // Unified reorder endpoint that accepts explicit order values for all item types
